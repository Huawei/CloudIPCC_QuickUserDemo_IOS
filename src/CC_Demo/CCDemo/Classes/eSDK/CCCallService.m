    ...
    switch (notifyEvent)
    {
        ...
        case CALL_E_EVT_CALL_OUTGOING:
        {
            if (self.isVediocall)
            {
                ...
                [self configLocalView:self.localViewWindow remoteView:self.remoteViewWindow callId:param1];
            }
            
        }
            
            //logDbg(@"call outgoing success %@",param1);
...
//                        if (self.isVediocall)
//                        {
//                            [self updateLocalView:self.localViewWindow remoteView:self.remoteViewWindow callId:self.callID];
//                        }
            [[CCUtil shareInstance] setVideoRotate:270];
             [[NSNotificationCenter defaultCenter] postNotificationName:@"deletelayer" object:nil];
            break;
...
- (void)startAnonymousCallWithAccessNumber:(NSString *)accessNumber andIp:(NSString *)ip andPort:(NSString *)port
{
    ...
    if ([LoginInfo sharedInstance].domain.length > 0)
    {
        callNum = [callNum stringByAppendingString:[NSString stringWithFormat:@"%@",[LoginInfo sharedInstance].domain]];
    }
    else
    {
...
- (void)configLocalView:(id)localView remoteView:(id)remoteView callId:(unsigned int)callId;
{
    ...
    logDbg(@"tup_call_set_video_window:%d callid:%d",windowRet,callId);
//    [[NSNotificationCenter defaultCenter] postNotificationName:CALL_MSG_ON_CONNECTED object:VIDEO_CALL];
  
    
}
- (void)updateLocalView:(id)localView remoteView:(id)remoteView callId:(unsigned int)callId;
{
    //视频窗口信息
    CALL_S_VIDEOWND_INFO videoInfo[2];
    memset_s(videoInfo, sizeof(CALL_S_VIDEOWND_INFO) * 2, 0, sizeof(CALL_S_VIDEOWND_INFO) * 2);
    ...
    TUP_RESULT windowRet = tup_call_update_video_window(2, videoInfo, callId);
    logDbg(@"tup_call_update_video_window:%d callid:%d",windowRet,callId);
